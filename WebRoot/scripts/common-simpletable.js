/*
* ! Simple Table. @author : hymer createTime : 2011-08-12
*/
...
var SimpleTable = function(tableId, options) {
	...
	var _default = {
		...
		listeners : {// all listeners must return a boolean value, determine
			// whether continue.
			oncheck : function(_self) {
				return true;
			},
			beforeload : function() {
				return true;
	...
	if(instance.table.length <= 0 || instance.table.get(0).tagName.toUpperCase() != 'TABLE') {
	...
	var refresh = function(instance, response) {
		if(response.result) {
			...
			// 由于已经转化成object，所以不用再fromJSON了，hymer modified @ 2012.6.1
	...
	var build = function(instance) {
		if(instance.table.attr("isbuilded"))
			return;
		instance.ckcol = [];
	...
	var buildPaging = function(instance) {
		if(instance.options.paging) {
			...
			if(instance.paging == null) {
				...
				buildSelectHtml(instance, pageSizeDiv);
				// select the pageSize.
				...
				go.click(function() {
					...
					if((goPage != instance.pageinfo.currentPage) && (1 <= goPage) && (goPage <= instance.pageinfo.totalPages)) {
			...
			if(instance.options.pagingOptions.go == false) {
				input.parent().hide();
				go.parent().hide();
			}
		}
	...
	var addEventListener = function(instance) {
		...
		if(instance.isAlreadyAddSortListener !== true && instance.options.sortables instanceof Array && instance.options.sortables.length > 0) {
	...
	var setCheckStatus = function(instance, head, cks) {
		...
		if(isSingle) {// single check, disable ckhead.
			...
			head.click(function() {
				...
				var checked = (checkedValue == true || 'checked' == checkedValue);
				$(cks).each(function(i) {
	...
	var addSortListener = function(instance, key) {
		for(var i = 0; i < instance.cols.length; i++) {
			...
			if(key == col.key) {
				col.element.addClass(instance.options.css.sortable).attr('sortable', true).click(function() {
